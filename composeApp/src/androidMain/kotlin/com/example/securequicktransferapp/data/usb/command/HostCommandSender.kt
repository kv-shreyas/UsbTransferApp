package com.example.securequicktransferapp.data.usb.command

import com.example.securequicktransferapp.data.usb.storage.UsbDataSource

import com.example.securequicktransferapp.data.usb.storage.SmartNavStorageResolver
...
class HostCommandSender @Inject constructor(
    ...
    suspend fun waitForClientReady(): Boolean = withContext(Dispatchers.IO) {
        ...
        for (i in 0 until 240) { // Try up to 240 times (60 seconds total) for remote device to enter Client mode and accept permission
            if (i % 2 == 0) {
                try {
                    dataSource.sendRawPacket(SecureQtSdk.Packets.TYPE_PING, ByteArray(0))
                } catch (e: Exception) {
                    usbLogger.w(TAG, "waitForClientReady: Failed to send ACK ping: ${e.message}")
                }
            }
            val packet = dataSource.receiveRawPacket()
            if (packet != null && packet.first == SecureQtSdk.Packets.TYPE_PING) {
            ...
            if (packet != null && packet.first == SecureQtSdk.Packets.TYPE_DATA) {
                usbLogger.w(TAG, "waitForClientReady: Discarding stale TYPE_DATA packet (${packet.second.size} bytes) received before commands started")
            }
            if (packet != null && packet.first != SecureQtSdk.Packets.TYPE_PING && packet.first != SecureQtSdk.Packets.TYPE_DATA) {
                usbLogger.w(TAG, "waitForClientReady: Ignoring unexpected packet type ${packet.first} during client status check")
            }
            if (i > 0 && i % 8 == 0) {
    ...
    suspend fun sendFile(localFile: File, remotePath: String, onProgress: suspend (Float) -> Unit = {}): Boolean = commandMutex.withLock {
        withContext(Dispatchers.IO + NonCancellable) {
            coroutineScope {
                ...
                if (success) {
                    dataSource.sendRawPacket(SecureQtSdk.Packets.TYPE_EOF, ByteArray(0))
                    usbLogger.i(TAG, "sendFile: Transfer completed successfully.")
                } else {
                    dataSource.sendRawPacket(SecureQtSdk.Packets.TYPE_CANCEL, ByteArray(0))
                    ...
                    try {
                        while (isActive) {
                            ...
                            if (packet.first == SecureQtSdk.Packets.TYPE_DATA) {
                                encryptedChannel.send(packet.second)
                            }
                        }
                    } finally {
                        encryptedChannel.close()
    ...
    suspend fun sendDisconnect() = withContext(Dispatchers.IO) {
        try {
            dataSource.sendSecure(byteArrayOf(SecureQtSdk.Commands.CMD_DISCONNECT))
        } catch (e: Exception) {
    ...
    override fun receiveStream(): kotlinx.coroutines.flow.Flow<ByteArray> = kotlinx.coroutines.flow.channelFlow {
        while (kotlinx.coroutines.currentCoroutineContext().isActive) {
            ...
            if (packet.first == SecureQtSdk.Packets.TYPE_DATA) {
                send(packet.second)
            } else if (packet.first == SecureQtSdk.Packets.TYPE_EOF) {
                break
            }
        }
    }
    ...
    override fun cancelTransfer() {
        kotlinx.coroutines.CoroutineScope(Dispatchers.IO).launch { dataSource.sendRawPacket(SecureQtSdk.Packets.TYPE_CANCEL, ByteArray(0)) }
    }
