...
import com.example.secureqt.sdk.SecureQtSdk.Packets as Packet
...
class UsbCommandProcessor @Inject constructor(
    ...
    suspend fun startListening(
        ...
        try {
            ...
            val ackJob = launch {
                for (i in 0 until 4) {
                    try {
                        dataSource.sendRawPacket(Packet.TYPE_PING, ByteArray(0))
                    } catch (e: Exception) {
                        break
                    }
    ...
    private fun listDirectFilesystem(dir: File): ArrayList<Triple<Boolean, Long, ByteArray>> {
        ...
        val pathStr = dir.absolutePath.trimEnd('/')
        if (pathStr == "/storage/emulated/0/Android" && !names.contains("data")) {
            items.add(Triple(true, 0L, "data".toByteArray(Charsets.UTF_8)))
        } else if (pathStr == "/storage/emulated/0/Android/data" && !names.contains("com.example.smartnav_v3")) {
            items.add(Triple(true, 0L, "com.example.smartnav_v3".toByteArray(Charsets.UTF_8)))
        } else if (pathStr == "/storage/emulated/0/Android/data/com.example.smartnav_v3" && !names.contains("files")) {
            items.add(Triple(true, 0L, "files".toByteArray(Charsets.UTF_8)))
        } else if (pathStr == "/storage/emulated/0/Android/data/com.example.smartnav_v3/files" && !names.contains("SmartNavV3")) {
            items.add(Triple(true, 0L, "SmartNavV3".toByteArray(Charsets.UTF_8)))
        }
            ...
            launch(Dispatchers.IO) {
                try {
                    while (isActive) {
                        ...
                        if (packet.first == Packet.TYPE_CANCEL) throw UsbDataSource.TransferCancelledException()
                        if (packet.first != Packet.TYPE_DATA) break
